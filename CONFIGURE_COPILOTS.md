...
| **Secret Location** (user auth) | Depends | When _User Authentication_ is checked, choose where you prefer to store the client secret. <br> Dataverse stores the secret in a secured column, while Key Vault requires to use an environment variable of type secret, and storing the secret in a Azure Key Vault. |
