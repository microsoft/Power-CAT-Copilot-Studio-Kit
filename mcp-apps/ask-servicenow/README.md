...
Use whichever option matches what you have. **Option A** if you received the `ask-servicenow.zip` package, **Option B** if you are cloning from source.

**Option A — Extract `ask-servicenow.zip`:**
```powershell
Expand-Archive -Path .\ask-servicenow.zip -DestinationPath .\ask-servicenow
cd ask-servicenow
```
