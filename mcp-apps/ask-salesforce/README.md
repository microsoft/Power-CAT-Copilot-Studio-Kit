...
Use whichever option matches what you have. **Option A** if you received the `ask-salesforce.zip` package, **Option B** if you are cloning from source.

**Option A — Extract `ask-salesforce.zip`:**
```powershell
Expand-Archive -Path .\ask-salesforce.zip -DestinationPath .\ask-salesforce
cd ask-salesforce
```
